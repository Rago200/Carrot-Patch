...
/**
 * This class tests the methods isClose, wolfEatRabbit, rabbitWatchForWolfMethod.
 */
public class CarrotPatchTester extends CarrotPatch {
  ...
  public static boolean test2isCloseMethod() {
    ...
    try {
      Wolf w1 = new Wolf();
      Wolf w2 = new Wolf();
      // Sets first wolf's position at (200,200)
      w1.setX(200);
      w1.setY(200);
      // Sets second wolf's position at (350, 200) (outside wolf's scan range)
      w2.setX(350);
      w2.setY(200);
      if (w1.isClose(w2, Wolf.getScanRange())) { // bug! should return false
        System.out.println(
                "Problem detected: Wolf.isClose() returned true when it should have returned " +
                        "false");
        return false;
      }
  ...
  public static boolean test3isCloseMethod() {
    ...
    try {
      Rabbit r = new Rabbit();
      // picks positions outside of Rabbit's scan range
      int xPosition = 200;
      int yPosition = 300;
      // Set rabbit position at (200, 200)
      r.setX(200);
      r.setY(100);
      if (r.isClose(xPosition, yPosition, Rabbit.getScanRange())) {
        System.out.println("Problem detected: Rabbit.isClose() returned true when it should have " +
                "returned false");
        return false;
      }
